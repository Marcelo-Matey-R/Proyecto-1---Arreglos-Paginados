...
class PagedArray{
    ...
    bool LoadPage(int idx, int pageNumber); //Carga la pagina del disco a la memoria en la posicion idx
    bool SavePage(int idx); //Carga la pagina pagedArr[idx] de la memoria al disco
    public:
    PagedArray(size_t pagedCount, size_t pagedSize, const std::string &filePath);
