...
class PagedArray{
    private:
    int freePos;
    std::unordered_map<long long, int> pages;
    size_t pagedCount;
    size_t pagedSize;
    long long totalInts;
    Page** pagedArr;
    LRUNode* head;
    LRUNode* tail;
    std::unordered_map<long long, LRUNode*> pageToNode;
    std::string filePath;
    long long pageFaults;
    long long pageHits;
    long long count;
    int FindPage(long long numberPaged); //Encuentra la existencia de la pagina que tenga dicho numero de pagina
    ...
    void PushFront(LRUNode* node); //envia al frente el nodo
    void RemoveNode(LRUNode* node); //elimina el nodo
    void MoveToFront(LRUNode* node); //mueve al frente el nodo
    int PopLRU(); //elimina y devuelve el nodo menos usado
    public:
